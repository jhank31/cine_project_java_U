...
public class ViewMovies extends JFrame implements ActionListener {
	...
	public ViewMovies(String datoCedula, String datoNombre) {
		...
		getPeli(((ArrayList<?>) asignarPelicula.getFuncions().get(0)).get(4),
				((ArrayList<?>) asignarPelicula.getFuncions().get(0)).get(3).toString(),
				((ArrayList<?>) asignarPelicula.getFuncions().get(0)).get(1).toString());
		getPeli(((ArrayList<?>) asignarPelicula.getFuncions().get(1)).get(4),
				((ArrayList<?>) asignarPelicula.getFuncions().get(1)).get(3).toString(),
				((ArrayList<?>) asignarPelicula.getFuncions().get(1)).get(1).toString());
		getPeli(((ArrayList<?>) asignarPelicula.getFuncions().get(2)).get(4),
				((ArrayList<?>) asignarPelicula.getFuncions().get(2)).get(3).toString(),
				((ArrayList<?>) asignarPelicula.getFuncions().get(2)).get(1).toString());
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (e.getSource() == capturar) {
			...
			if (datosPersona.estaEnBaseDatos(datoTextField.trim())) {
				...
				ModelUser.setSaldoActualPer(datosPersona.getSaldo(datoTextField.trim()));
				JOptionPane.showMessageDialog(null, "Inicio Sesion Correcto");

				datosPersona.imprimir();
